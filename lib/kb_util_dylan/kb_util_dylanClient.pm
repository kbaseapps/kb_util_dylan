...
<pre>
$params is a kb_util_dylan.KButil_Extract_Unpaired_Reads_Params
$return is a kb_util_dylan.KButil_Extract_Unpaired_Reads_Output
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_util_dylan.KButil_Extract_Unpaired_Reads_and_Synchronize_Pairs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$params is a kb_util_dylan.KButil_Build_AssemblySet_Params
$return is a kb_util_dylan.KButil_Build_AssemblySet_Output
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_util_dylan.KButil_Build_AssemblySet",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
